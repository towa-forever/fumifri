...
// ---- AI機能（サーバー側でAnthropic APIを呼び出す）----
async function callClaude(system, userText, maxTokens) {
  if (!process.env.ANTHROPIC_API_KEY) {
    const err = new Error('AI機能が設定されていません（ANTHROPIC_API_KEY未設定）');
  ...
  if (data.error) throw new Error(data.error.message || 'AI呼び出しエラー');
  return (data.content || []).map(b => b.text || '').join('').trim();
}

// 文房具専用AIチャット
app.post('/api/ai/stationery', async (req, res) => {
  const { question } = req.body;
  if (!question || !question.trim()) return res.status(400).json({ error: '質問を入力してください' });
  try {
    const answer = await callClaude(
      'あなたは「文具市場」という文房具フリマアプリの中にいる、文房具に詳しいフレンドリーなアシスタントです。ペン・ノート・消しゴム・定規・ファイル用品など、文房具に関する質問（選び方、使い方、お手入れ方法、比較、豆知識など）に、親しみやすい口調でわかりやすく答えてください。文房具と関係のない質問（雑談、他の商品カテゴリ、個人的な相談など）をされた場合は、やんわりと「文房具に関する質問なら何でも聞いてください！」と伝えて断ってください。回答は短めに、3〜5文程度でまとめてください。',
      question.trim(),
      400
    );
    res.json({ answer });
...
// AIクーポン生成
app.post('/api/ai/coupon', async (req, res) => {
  const { productName, price } = req.body;
  if (!productName || !price) return res.status(400).json({ error: '必須項目が足りません' });
  try {
    const text = await callClaude(
      '文房具フリマアプリのクーポン生成アシスタントです。指定された商品にお得なクーポンを1つ考えて、JSON形式だけで答えてください。説明文やコードブロックは付けないでください。',
      `商品「${productName}」が${price}円で売られています。購入者へのお得なクーポン割引を考えてください。出力例: {"type":"percent","value":10,"reason":"初回購入特典"}\ntypeはamount（円引き）かpercent（%OFF）、valueは数字のみ、reasonは10文字程度の理由。`,
      120
    );
